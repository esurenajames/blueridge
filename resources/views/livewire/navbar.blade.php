...
<div class="py-2 px-6 bg-[#f8f4f3] flex items-center shadow-md shadow-black/5 sticky top-0 left-0 z-30">   
    ...
    <ul class="ml-auto flex items-center ">
        ...
        <li class="dropdown">
            <button type="button" class="dropdown-toggle text-gray-400 mr-4 w-8 h-8 rounded flex items-center justify-center hover:text-gray-600 relative">
                @if($unreadCount > 0)
            ...
            <div class="dropdown-menu shadow-md shadow-black/5 z-30 hidden max-w-sm w-full bg-white rounded-md border border-gray-300">
                        ...
                        @foreach($notifications as $notification)
                            <li>
                                <a href="#" class="py-2 px-4 flex items-center hover:bg-gray-50 group">
                                    ...
                                    <div class="ml-2">
                                        <div class="text-[14px] text-gray-600 font-medium line-clamp-2 group-hover:text-blue-500 {{ $notification->is_read ? 'text-gray-400' : 'font-bold' }}">
                                            {{ $notification->message }}
                                        </div>
                                        <div class="text-[12px] text-gray-400">{{ $notification->created_at->diffForHumans() }}</div>
                                    </div>
                                    @if (!$notification->is_read)
                                        <span class="absolute right-4 bg-red-600 text-white text-xs font-semibold rounded-full w-4 h-4 flex items-center justify-center">!</span>
                                    @endif
                                </a>
                            </li>
                        @endforeach
                        <div class="flex justify-center py-1">
                            <button wire:click="viewAll" class="text-gray-600 text-[14px] hover:text-black">View All</button>
                        </div>
                    </ul>
                </div>
            </div>
