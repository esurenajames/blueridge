...
<body>
    ...
    <div class="flex flex-col md:flex-row ml-5 mr-10">
                    ...
                    <form action="{{ route('profile.update') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="grid mt-8" x-data="{ open: false, showModal: false }">
                            <div class="relative flex flex-col items-center space-y-3 sm:flex-row sm:space-y-0">
                                <img @click="open = !open"
                                     class="object-cover w-40 h-40 p-1 rounded-full ring-2 ring-indigo-300 dark:ring-indigo-500 cursor-pointer"
                                     src="{{ Auth::user()->profile_picture ? Storage::url(Auth::user()->profile_picture) : asset('images/default-profile.jpg') }}"
                                     alt="Bordered avatar">
                                <div x-show="open" @click.away="open = false"
                                     class="absolute top-44 mt-2 bg-white border dark:bg-gray-800 border-gray-700 shadow-lg rounded-md w-55 z-10">
                                    <ul>
                            ...
                            <div class="items-center mt-4 sm:mt-8 text-[#202142]">
                                <div class="flex flex-col items-center w-full mb-2 space-x-0 space-y-2 sm:flex-row sm:space-x-4 sm:space-y-0 sm:mb-6">
                                    <div class="w-full">
                                        ...
                                        <input type="text" id="fname" name="fname" value="{{ Auth::user()->fname ?? '' }}" class="bg-gray-50 border border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-gray-500 focus:border-gray-500 block w-full p-2.5 outline-none" required>
                                    </div>
    
                                    <div class="w-full">
                                        <label for="last_name" class="block mb-2 text-sm font-medium text-indigo-900 dark:text-black">Last name</label>
                                        <input type="text" id="lname" name="lname" value="{{ Auth::user()->lname ?? '' }}" class="bg-gray-50 border border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-gray-500 focus:border-gray-500 block w-full p-2.5 outline-none" required>
                                    </div>
                                </div>
    
                                <div class="mb-2 sm:mb-6">
                                    <label for="email" class="block mb-2 text-sm font-medium text-indigo-900 dark:text-black">Username</label>
                                    <input type="text" id="username" name="username" value="{{ Auth::user()->username ?? '' }}" class="bg-gray-50 border border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-gray-500 focus:border-gray-500 block w-full p-2.5 outline-none" required>
                                </div>
    
                                <div class="mb-2 sm:mb-6">
                                    <label for="email" class="block mb-2 text-sm font-medium text-indigo-900 dark:text-black">Email</label>
                                    <input type="email" id="email" name="email" value="{{ Auth::user()->email ?? '' }}" class="bg-gray-50 border border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-gray-500 focus:border-gray-500 block w-full p-2.5 outline-none" required>
                                </div>
    
                                <div class="mb-2 sm:mb-6">
                                    <label for="profession" class="block mb-2 text-sm font-medium text-indigo-900 dark:text-black">Profession</label>
                                    <input type="text" id="profession" name="profession" value="{{ Auth::user()->profession ?? '' }}" class="bg-gray-50 border border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-gray-500 focus:border-gray-500 block w-full p-2.5 outline-none" placeholder="Barangay Official" required>
                                </div>
                    ...
                    <form action="{{ route('password.update') }}" method="POST">
                        ...
                        <div class="grid mt-8">
                            <div class="mb-2 sm:mb-6">
                                ...
                                <input type="password" id="current_password" name="current_password" class="bg-gray-50 border border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-gray-500 focus:border-gray-500 block w-full p-2.5 outline-none" placeholder="Enter current password" required>
                            </div>
    
                            <div class="mb-2 sm:mb-6">
                                <label for="new_password" class="block mb-2 text-sm font-medium text-indigo-900 dark:text-black">New Password</label>
                                <input type="password" id="new_password" name="new_password" class="bg-gray-50 border border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-gray-500 focus:border-gray-500 block w-full p-2.5 outline-none" placeholder="Enter new password" required>
                            </div>
                            ...
                            <div class="mb-2 sm:mb-6">
                                ...
                                <input type="password" id="retype_password" name="new_password_confirmation" class="bg-gray-50 border border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-gray-500 focus:border-gray-500 block w-full p-2.5 outline-none" placeholder="Retype new password" required>
                            </div>
                        </div>
                        <div class="flex justify-end w-full">
                            <button type="submit" class="text-white bg-indigo-700 hover:bg-indigo-800 focus:ring-4 focus:outline-none focus:ring-indigo-300 font-medium rounded-lg text-sm w-full px-5 py-2.5 text-center dark:bg-indigo-600 dark:hover:bg-indigo-700 dark:focus:ring-indigo-800 mt-2">Save</button>
        ...
        <!-- Error Modal -->
        <div x-data="{ showErrorModal: @if(session()->has('error')) true @else false @endif }">
            <div x-show="showErrorModal" class="fixed inset-0 px-4 flex flex-wrap justify-center items-center w-full h-full z-[100] before:fixed before:inset-0 before:w-full before:h-full before:bg-[rgba(0,0,0,0.5)] overflow-auto font-[sans-serif]">
                <div class="fixed inset-0 flex items-center justify-center">
                    <div class="bg-white rounded-lg shadow-lg p-6 max-w-md w-full">
                        <div class="text-center mt-8">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-16 fill-current text-[#333] inline-block mb-4" viewBox="0 0 512 512">
                                <path d="M256 0C114.836 0 0 114.836 0 256s114.836 256 256 256 256-114.836 256-256S397.164 0 256 0zm0 470.487c-118.265 0-214.487-96.214-214.487-214.487 0-118.265 96.221-214.487 214.487-214.487 118.272 0 214.487 96.221 214.487 214.487 0 118.272-96.215 214.487-214.487 214.487z"/>
                                <path d="M256 144.48c-11.046 0-20 8.954-20 20v124.078c0 11.046 8.954 20 20 20s20-8.954 20-20V164.48c0-11.046-8.954-20-20-20z"/>
                                <circle cx="256" cy="362.29" r="21"/>
                            </svg>
                            <h4 class="text-2xl text-[#333] font-semibold mt-6">{{ session('error') }}</h4>
                            <p class="text-sm text-gray-500 mt-4">{{ session('error_message') }}</p>
                        </div>
                        <button @click="showErrorModal = false" class="bg-[#333] hover:bg-[#222] text-white text-sm font-semibold rounded-full px-6 py-2.5 mt-8 w-full focus:outline-none">Okay</button>
                    </div>
                </div>
            </div>
        </div>
